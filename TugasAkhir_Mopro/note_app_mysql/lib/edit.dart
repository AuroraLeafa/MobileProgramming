...
class _EditState extends State<Edit> {
  ...
  Future _getData() async {
    try {
      final response = await http.get(
          //Ubah IP pada Uri.Parse menjadi IP pribadi, karena jika menggunakan "localhost/" akan menghasilkan error
          //get detail data with id
        Uri.parse("http://192.101.101.1/latihan/note_app/detail.php?id='${widget.id}'"));
  ...
  Future _onUpdate(context) async {
    try {
      return await http.post(
        //Ubah IP pada Uri.Parse menjadi IP pribadi, karena jika menggunakan "localhost/" akan menghasilkan error
        Uri.parse("http://192.101.101.1/latihan/note_app/update.php"),
        body: {
          "id": widget.id,
          "title": title.text,
  ...
  Future _onDelete(context) async {
    try {
      return await http.post(
        //Ubah IP pada Uri.Parse menjadi IP pribadi, karena jika menggunakan "localhost/" akan menghasilkan error
        Uri.parse("http://192.101.101.1/latihan/note_app/delete.php"),
        body: {
          "id": widget.id,
        },
      ).then((value) {
